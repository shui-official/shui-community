...
export default function QuestPanel() {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur">
      ...
      {pts ? (
        <div className="mt-2 grid gap-2 md:grid-cols-3 text-xs text-white/60">
          <div className="rounded-xl border border-white/10 bg-black/20 px-3 py-2">
            {safeText(t("quests.pointsActivity", "Activité"), "Activité")} :{" "}
            <span className="text-white/80 font-semibold">{pts.activity}</span>
          </div>
          <div className="rounded-xl border border-white/10 bg-black/20 px-3 py-2">
            {safeText(t("quests.pointsOnchain", "On-chain"), "On-chain")} :{" "}
            <span className="text-white/80 font-semibold">{pts.onchain}</span>
          </div>
          <div className="rounded-xl border border-white/10 bg-black/20 px-3 py-2">
            {safeText(t("quests.pointsTotalShort", "Total"), "Total")} :{" "}
            <span className="text-white/80 font-semibold">{pts.total}</span>
          </div>
        </div>
      ) : null}
      ...
      <div className="mt-4 grid gap-3">
        ...
        {data?.ok && !data.quests.length ? (
          <div className="text-sm text-white/60">
            {safeText(t("quests.empty", "Aucune quête disponible."), "Aucune quête disponible.")}
          </div>
        ) : null}

        {!data ? (
          <div className="text-sm text-white/60">
            {safeText(t("loading", "Chargement…"), "Chargement…")}
          </div>
        ) : null}
      </div>
      ...
      <p className="mt-4 text-xs text-white/50">
        {safeText(
          t("quests.securityNote", "Sécurité : endpoints protégés par session + allowlist “quest-claim” + rate-limit."),
          "Sécurité : endpoints protégés par session + allowlist “quest-claim” + rate-limit."
        )}
      </p>
    </div>
  );
