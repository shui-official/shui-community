...
export const HomeView: FC = () => {
  ...
  // ✅ SHUI balance (added)
  const [shuiBalance, setShuiBalance] = useState<number | null>(null);
  const [loadingShui, setLoadingShui] = useState(false);
  const [shuiError, setShuiError] = useState<string | null>(null);

  // ✅ Load SHUI balance when wallet connects/disconnects (added)
  useEffect(() => {
    let cancelled = false;

    async function loadShui() {
      try {
        setShuiError(null);
        ...
        const bal = await getShuiBalance(owner);

        if (!cancelled) setShuiBalance(bal);
      } catch (e: any) {
        console.error("SHUI balance error:", e);
        if (!cancelled) {
          setShuiBalance(null);
          setShuiError(e?.message || "Failed to load SHUI balance");
        }
      } finally {
        if (!cancelled) setLoadingShui(false);
      }
    }
