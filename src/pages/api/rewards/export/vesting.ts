...
import { buildVestingCsvRows, toCsvString } from "../../../../lib/rewards/csv";
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const rows = buildVestingCsvRows(ledger, period);
    const csv = toCsvString(rows);

    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename="shui-vesting-${period}.csv"`);
